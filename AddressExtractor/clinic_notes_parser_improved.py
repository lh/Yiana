#!/usr/bin/env python3
"""
Improved Clinic Notes Parser
Extracts patient information including titles from structured clinic notes
"""
...
@dataclass
class PatientNote:
    """Parsed patient note from clinic with title support"""
    spire_mrn: str
    patient_name: str
    patient_title: Optional[str]  # Mr, Mrs, Ms, Miss, Dr, etc.
    gp_name: str
...
class ImprovedClinicNotesParser:
    """Parse clinic notes with proper title extraction"""
    ...
    def __init__(self):
        ...
        # Title patterns
        self.title_patterns = [
            'Mr', 'Mrs', 'Ms', 'Miss', 'Master', 'Dr', 'Prof', 'Professor',
            'Rev', 'Reverend', 'Sir', 'Dame', 'Lady', 'Lord'
        ]
    ...
    def extract_title_and_name(self, name_line: str) -> Tuple[Optional[str], str]:
        """Extract title and name from a line"""
        name_line = name_line.strip()
        
        # Check for title at the beginning
        title = None
        for t in self.title_patterns:
            if name_line.startswith(t + ' '):
                title = t
                name_line = name_line[len(t):].strip()
                break
        
        return title, name_line
    ...
    def parse_single_note(self, text: str) -> Optional[PatientNote]:
        ...
        if mrn_line_idx < len(lines) - 1:
            name_line = lines[mrn_line_idx + 1].strip()
            patient_title, patient_name = self.extract_title_and_name(name_line)
    ...
    def parse_batch_notes(self, text: str) -> List[PatientNote]:
        ...
        for section in sections:
            ...
            if has_mrn:
                ...
                if note:
                    notes.append(note)
                    logger.info(f"Parsed note for {note.patient_title or ''} {note.patient_name} (MRN: {note.spire_mrn})")
    ...
    def format_for_database(self, note: PatientNote) -> Dict:
        """Format parsed note for database insertion with title"""
...
def main():
    """Test the improved parser with title extraction"""
    import argparse
    
    parser = argparse.ArgumentParser(description='Parse clinic notes with titles')
    ...
    notes_parser = ImprovedClinicNotesParser()
    
    if args.test:
        # Test with sample data including titles
        sample_notes = """
0030730605
Mrs Patricia Wheatley
Doctor E Robinson
...
"""
        ...
        for i, note in enumerate(notes, 1):
            print(f"{i}. Patient: {note.patient_title or ''} {note.patient_name}")
            print(f"   Title: {note.patient_title}")
        ...
        if notes:
            ...
            for i, note in enumerate(notes, 1):
                print(f"{i}. {note.patient_title or ''} {note.patient_name} (MRN: {note.spire_mrn})")
                print(f"   Title: {note.patient_title}")
                print(f"   GP: {note.gp_name}")
                if note.optician_name:
                    print(f"   Optician: {note.optician_name}")
                ...
                print(f"   DB format: {db_data.get('title', '')} {db_data['first_name']} {db_data['last_name']}")
                print()
        else:
        ...
        parser.print_help()
        print("\nExample usage:")
        print("  python clinic_notes_parser_improved.py --test")
        print("  python clinic_notes_parser_improved.py clinic_notes.txt")
