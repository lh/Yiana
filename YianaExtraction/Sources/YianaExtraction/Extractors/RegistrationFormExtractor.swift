...
public struct RegistrationFormExtractor: Extractor {
    ...
    public func extract(from input: ExtractionInput) -> AddressPageEntry? {
        ...
        // Step 2: MRN
        let mrn = ExtractionHelpers.firstMatch(#"Patient_?\s*(\d{6,10})"#, in: text)?[1]
            ?? ExtractionHelpers.firstMatch(#"Patient\s*No\.?\s*(\d{6,10})"#, in: text)?[1]
        ...
        if fullName == nil, let m = ExtractionHelpers.firstMatch(
            #"([A-Z][a-z]+,\s*[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\s*\n\s*(\d{1,2}[./]\d{1,2}[./]\d{4})"#,
            in: text, options: [.caseInsensitive]
        ) {
            fullName = flipName(m[1])
        ...
        if dob == nil {
            ...
            for pattern in dobPatterns {
                if let m = ExtractionHelpers.firstMatch(pattern, in: text, options: [.caseInsensitive]) {
        ...
        let phoneMatches = ExtractionHelpers.allMatches(#"(\d{5}\s*\d{6}|\d{11})"#, in: patientText)
